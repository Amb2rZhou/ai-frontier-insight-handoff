...
AI Frontier Insight Bot 是一个 **AI 前沿情报系统**，每天自动采集多源信息、用 LLM 提取关键信号、生成带洞察的日报，通过 Hi 群分发。

v1 稳定运行 2 个多月（2026-02-25 至今），日报从未中断。跑在原维护者的 Mac 上，launchd 定时触发。
...
│ Twitter (200+)│────▶│ 最有价值信号  │────▶│ 洞察 + 启示  │────▶│ Git push     │
│ GitHub (10)   │     │ 去重 + 过滤  │     │ 趋势总结     │     │ → Seal 拉取   │
│ ArXiv (25)    │     └──────────────┘     └──────────────┘     │ → Redoc      │
│ HuggingFace   │                                               │ → Hi 群      │
│ Benchmarks    │                                               └──────────────┘
└──────────────┘
...
| 9 | Seal（外部） | 从 GitHub 拉取新日报 → 写入 Redoc 空间 → webhook 发 Redoc 链接到 Hi 群 |
...
Pipeline 每天生成 insight 后，会自动把相关条目写入 `wiki/` 下对应的实体页面时间线。

`src/wiki/updater.py` 中有一个 `ENTITY_MAP`（关键词 → wiki 页面的映射表），匹配到的 insight 会追加到对应页面的 `## Timeline` 下。新增实体需手动在 `ENTITY_MAP` 中添加映射。

目前 wiki 仅用于**写入**（自动追加时间线），没有查询能力。v2 计划将 wiki 升级为可交互的知识库，详见 Part 2。
...
v1 的 wiki 只做自动时间线追加。v2 的核心升级是把 wiki 变成**可查询的结构化知识库**。
...
```
云端 Agent（内部服务器）
  ├── 数据采集：X API (按量付费) + RSS + ArXiv + GitHub + HuggingFace
  ├── LLM-Wiki：自动维护结构化知识库
...
| Twitter 数据 | Playwright 浏览器爬虫（不稳定） | X API 按量付费（~$66/月） |
| LLM 模型 | DeepSeek V3（外部 API） | 内部模型 API 或 agent 调用 |
| 分发 | 本地 → GitHub → Seal → Redoc → Hi | 云端直接 → Seal 读本地文件 → Redoc → Hi |
...
- [ ] 配置分发：Seal 读取本地日报文件 → 写入 Redoc → webhook 发链接到 Hi 群（不再需要 GitHub 中转）
- [ ] 验证 pipeline 端到端运行
...
| `src/delivery/webhook.py` | 早期直推方案遗留 | Seal 分发或重新启用 |
...
| `x-monitor/data/list_members.json` | X 账号完整列表（207 个） | ✅ |
| `wiki/` | LLM-Wiki 知识库（60+ 页） | ✅ |
| `memory/trends.json` | 趋势追踪数据 | ✅ |
| `data/weekly/` | 8 期周报 | ✅ |
| `docs/handoff-guide.md` | 本文档 | ✅ |
| `docs/v2-design.md` | v2 架构设计 | ✅ |
